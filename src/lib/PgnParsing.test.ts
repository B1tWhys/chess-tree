...
function moveNodeFromPgn(pgn: string): MoveNode | null {
	const tree = GameTree.fromPgnStr(pgn);
	return tree.rootMoves.length ? tree.rootMoves[0] : null;
}

function minimalMoveObject(name: string, isWhiteTurn: boolean, children: Array<unknown>) {
	return {
		name: name,
...
describe('When creating a game tree from PGN', function () {
	...
	test('A 1 ply game should return a root node with no children', () => {
		const result = moveNodeFromPgn('1. d3 *');
	...
	test('A linear 2 ply game returns a root & single child', () => {
		const result = moveNodeFromPgn('1. d3 d6 *');
		...
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [minimalMoveObject('d6', false, [])])
		);
	});

	test('A linear 3 ply game returns a root with a child with a child', () => {
		const result = moveNodeFromPgn('1. d3 d6 2. e4 *');

		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
				minimalMoveObject('d6', false, [minimalMoveObject('e4', true, [])])
			])
		);
	});

	test('A game with 2 variations of ply 2 is handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 (1... e6) *');
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
	...
	test('A game with 3 variations of ply 2 is handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 (1... e6) (1... f6) *');
	...
	test('A tree with 2 variations at ply 3 are handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 2. e3 (2. e4) *');
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
				minimalMoveObject('d6', false, [
	...
	test('A multi-ply variation is handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 (1... e6 2. e4 f6) *');
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
				minimalMoveObject('d6', false, []),
				minimalMoveObject('e6', false, [
					minimalMoveObject('e4', true, [minimalMoveObject('f6', false, [])])
				])
			])
		);
	});
	...
	test('Singly nested variations of ply 2 are handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 (1... e6 2. e4 f6 (2... f5 3. exf5) 3. g3) *');
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
				minimalMoveObject('d6', false, []),
				minimalMoveObject('e6', false, [
					minimalMoveObject('e4', true, [
						minimalMoveObject('f6', false, [minimalMoveObject('g3', true, [])]),
						minimalMoveObject('f5', false, [minimalMoveObject('exf5', true, [])])
					])
				])
			])
		);
	...
	test('Singly nested variations of ply 3 are handled properly', () => {
		const result = moveNodeFromPgn('1. d3 d6 2. e4 (2. e3 f6 (2... f5) 3. f4) *');
		expect(result).toMatchObject(
			minimalMoveObject('d3', true, [
				minimalMoveObject('d6', false, [
					minimalMoveObject('e4', true, []),
					minimalMoveObject('e3', true, [
						minimalMoveObject('f6', false, [minimalMoveObject('f4', true, [])]),
...
describe('When converting a move tree to PGN', function () {
	...
	test('A complicated game tree is encoded properly', () => {
		const pgnStr =
			'1. d4 (1. a4 a5 2. b4 (2. c3 c6 (2... e5 3. e4 f6)) 2... b5) 1... d5 2. e4 e5 (2... f6 3. g3 g5) 3. f4 f5 *';
		const result = GameTree.fromPgnStr(pgnStr).toPgn();
...
describe('When merging game trees', function () {
	...
	test('Deeply nested variations are merged', () => {
		...
		const tree3 = GameTree.fromPgnStr(
			'1. a3 (1. b3) 1... a6 (1... b6 2. b3 (2. c3 c6 (2... f6) (2... g6)) (2. e3)) *'
		);
		const merged = GameTree.merge(tree1, tree2, tree3);
		const expected =
			'1. a3 (1. b3) 1... a6 (1... b6 2. b3 (2. c3 c6 (2... d6) (2... e6) (2... f6) (2... g6)) (2. d3) (2. e3)) *';
